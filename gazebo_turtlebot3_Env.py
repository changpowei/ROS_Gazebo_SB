...
class Turtlebot3GymEnv():
    ...
    def __init__(self):
        ...
        self.laserMinRange = 0.15  # Modify laser data and fix min range to
        ...
        self.minimum_step_reward = -20

        self.CarPositionChange = True
    ...
    def Depthcallback(self, msg_depth):
        ...
        try:
            ...
            """
            image = np.frombuffer(msg_depth.data, dtype=np.float32).reshape(msg_depth.height, msg_depth.width, -1)
            #image = cv_bridge.imgmsg_to_cv2(msg_depth, "32FC1")

            # Resize to the desired size
            image_resized = cv2.resize(image, (192, 108), interpolation=cv2.INTER_CUBIC)[:89, :]
    ...
    def _compute_reward(self, distanceToTarget, isCrash):

        reward = self.time_penalty
        self.CarPositionChange = False
        done = False
        ...
        # 如過比上次更靠近目標就加分，反之就扣分，放大5倍
        reward_dist_to_target = (self.oldDistance - distanceToTarget) * 5
    ...
    def step(self, action):
        ...
        done, reward = self._compute_reward(distanceToTarget = distanceToTarget, isCrash=isCrash)
